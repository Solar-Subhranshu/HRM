...
const monthlyResignationReport = async(req,res)=>{
    try {
        const {monthDate} = req.query || req.body;
        ...
        const dbData = await Employee.find({
            // status:"Approved",
            regisnationDate: { $gte: firstDayOfMonth, $lte: lastDayOfMonth },
        })
        .populate({
            path:"department",
            select:"department"
        ...
        if(dbData.length===0){
            return res.status(202).json({
                success:true,
                message:"No Resignation found for the selected month.",
                data:[]
            })
        }
        // console.log(dbData);
        ...
        const refinedDataArr = dbData.map((item)=>({
            ...
            Reporting : item.reportingManager.name,
            Joining_Date : moment(item.joiningDate).format("DD-MM-YYYY"),
            Resignation_Date: moment(item.regisnationDate).format("DD-MM-YYYY"),
            // Bank_Name : item.bankName,
            // Bank_IFSC_Code : item.bankIFSC,
            // Bank_Account_No : `${item.bankAccount},`,
            // Aadhar_No : `${item.aadharCard},`,
        ...
        const workbook = xlsx.utils.book_new();
        const worksheet = xlsx.utils.json_to_sheet(refinedDataArr);
        xlsx.utils.book_append_sheet(workbook,worksheet,"Resignation-Report");

        const sendableBuffer = xlsx.write(workbook,{bookType:'xlsx', type:'buffer'});
        const fileName = `${moment(correctMonthDate).format("MMM-YYYY")} Resignation Report.xlsx`;
        res.setHeader('Content-Disposition', `attachment; filename="${fileName}"`);
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
