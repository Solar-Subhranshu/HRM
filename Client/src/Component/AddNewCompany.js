...
function AddNewCompany() {
  ...
  const saveNewCompanyName = async (e) => {
    ...
    // admin@002
    try {
       const response= await axios.post("http://localhost:8000/company/add-company", {
        name: inputData.name,
      }
    );
  ...
  const saveNewBranch = async (e) => {
    ...
    try {
      const response=await axios.post("http://localhost:8000/branch/add-branch", {
  ...
  const fetchAllCompanyDetails=async ()=>{
   try {
    const response=await axios.get('http://localhost:8000/branch/show-all-companyDetails');
    ...
    <div className="ml-5 mr-5">
      ...
      <div>
        <table className="table-auto w-full border border-black">
          ...
          <tbody>
            {showAllCompanyDetails.map(({_id,address,name,companyID})=>(
              <tr key={_id}>
